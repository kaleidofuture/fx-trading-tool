...
def main():
    ...
    # ── トレード記録タブ ──
    with tab_trades:
        with st.expander(L["new_trade"], expanded=False):
            with st.form("new_trade_form"):
                fc1, fc2, fc3 = st.columns(3)
                new_pair = fc1.selectbox(L["pair_select"], list(PAIRS.keys()), key="new_pair")
                new_direction = fc2.selectbox(L["direction"], [L["long"], L["short"]], key="new_dir")
                new_units = fc3.number_input(L["units"], min_value=1, value=1000, step=1000, key="new_units")

                fc4, fc5, fc6 = st.columns(3)
                new_entry = fc4.number_input(L["entry_price"], min_value=0.0, value=0.0, format="%.5f", key="new_entry")
                new_sl = fc5.number_input(L["sl"], min_value=0.0, value=0.0, format="%.5f", key="new_sl")
                new_tp = fc6.number_input(L["tp"], min_value=0.0, value=0.0, format="%.5f", key="new_tp")

                new_note = st.text_input(L["memo"], key="new_note")

                if st.form_submit_button(L["submit_trade"], type="primary"):
                    if new_entry > 0 and new_sl > 0 and new_tp > 0:
                        direction = "LONG" if "LONG" in new_direction else "SHORT"
                        trade = {
                            "id": next_trade_id(trades), "pair": new_pair, "direction": direction,
                            "entry_price": new_entry, "units": new_units, "stop_loss": new_sl,
                            "take_profit": new_tp, "entry_time": datetime.now(timezone.utc).isoformat(),
                            "exit_price": None, "exit_time": None, "pnl_jpy": None,
                            "status": "OPEN", "note": new_note,
                        }
                        trades.append(trade)
                        save_trades(trades)
                        st.success(f"Trade #{trade['id']} {L['trade_recorded']}: {new_pair} {direction} @ {new_entry}")
                        st.rerun()
                    else:
                        st.error(L["enter_price"])

        open_trades = [t for t in trades if t["status"] == "OPEN"]
        if open_trades:
            st.markdown(f"### {L['open_pos']} ({len(open_trades)})")
            for t in open_trades:
                ...
                with st.container():
                    st.markdown(
                        ...
                        f'<strong>#{t["id"]} {arrow} {t["pair"]} {t["direction"]}</strong> @ {t["entry_price"]}'
                        f'<br><small>SL: {t["stop_loss"]}  |  TP: {t["take_profit"]}  |  '
                        f'Units: {t["units"]:,}</small>'
                        f'{"<br><small>" + L["memo"].split(" ")[0] + ": " + t["note"] + "</small>" if t.get("note") else ""}'
                        f'</div>',
                        unsafe_allow_html=True,
                    )
                    with st.expander(f"Trade #{t['id']} {L['close_trade']}"):
                        with st.form(f"close_form_{t['id']}"):
                            cc1, cc2 = st.columns(2)
                            close_price = cc1.number_input(
                                L["exit_price"], min_value=0.0, value=0.0, format="%.5f",
                                key=f"close_price_{t['id']}"
                            )
                            close_reason = cc2.selectbox(
                                L["exit_reason"],
                                [L["reason_tp"], L["reason_sl"], L["reason_manual"], L["reason_other"]],
                                key=f"close_reason_{t['id']}"
                            )
                            if st.form_submit_button(L["close_btn"], type="primary"):
                                if close_price > 0:
                                    t["exit_price"] = close_price
                                    t["exit_time"] = datetime.now(timezone.utc).isoformat()
                                    t["status"] = "CLOSED"
                                    t["exit_reason"] = close_reason.split(" ")[0]
                                    t["pnl_jpy"] = calc_pnl_jpy(
                                        t["pair"], t["direction"], t["entry_price"], close_price, t["units"]
                                    )
                                    save_trades(trades)
                                    result = L["win"] if t["pnl_jpy"] > 0 else L["lose"]
                                    st.success(f"Trade #{t['id']}: {t['pnl_jpy']:+,.0f} 円 ({result})")
                                    st.rerun()
                                else:
                                    st.error(L["enter_exit_price"])
        elif trades:
            st.info(L["no_open"])

        closed_trades = [t for t in trades if t["status"] == "CLOSED"]
        if closed_trades:
            st.markdown(f"### {L['closed_trades']} ({len(closed_trades)})")
            df_closed = pd.DataFrame(closed_trades)
            display_cols = ["id", "pair", "direction", "entry_price", "exit_price", "pnl_jpy", "exit_reason"]
            available_cols = [c for c in display_cols if c in df_closed.columns]
            st.dataframe(
                df_closed[available_cols].style.map(
    ...
    with tab_report:
        ...
        if not closed:
            st.info(L["no_closed"])
            st.markdown(f"### {L['backtest_reports']}")
            for img_name in ["USDJPY_backtest.png", "EURUSD_backtest.png"]:
                img_path = REPORTS_DIR / img_name
                if img_path.exists():
                    st.image(str(img_path), caption=img_name.replace("_", " ").replace(".png", ""))
        else:
            total = len(closed)
            wins = len([t for t in closed if t["pnl_jpy"] > 0])
            losses = len([t for t in closed if t["pnl_jpy"] < 0])
            total_pnl = sum(t["pnl_jpy"] for t in closed)
            total_win = sum(t["pnl_jpy"] for t in closed if t["pnl_jpy"] > 0)
            total_loss = abs(sum(t["pnl_jpy"] for t in closed if t["pnl_jpy"] < 0))
            pf = total_win / total_loss if total_loss > 0 else float("inf")

            st.markdown(f"### {L['demo_performance']}")
            m_cols = st.columns(5)
            m_cols[0].metric(L["total_trades"], total)
            m_cols[1].metric(L["win_rate"], f"{wins/total*100:.1f}%")
            m_cols[2].metric(L["profit_factor"], f"{pf:.2f}")
            m_cols[3].metric(L["total_pnl"], f"{total_pnl:+,.0f} 円")
            m_cols[4].metric("W/L", f"{wins}/{losses}")

            eq_fig = create_equity_chart(trades, L)
            if eq_fig:
                st.markdown(f"### {L['equity_curve']}")
                st.plotly_chart(eq_fig, use_container_width=True)

            st.markdown(f"### {L['by_pair']}")
            for pair_name in PAIRS:
                pair_closed = [t for t in closed if t["pair"] == pair_name]
                if pair_closed:
                    pw = len([t for t in pair_closed if t["pnl_jpy"] > 0])
                    pp = sum(t["pnl_jpy"] for t in pair_closed)
                    st.markdown(
                        f"**{pair_name}**: {len(pair_closed)} {L['trades_label']}, "
                        f"{L['win_rate']} {pw/len(pair_closed)*100:.0f}%, "
                        f"PnL {pp:+,.0f} 円"
                    )

            st.markdown(f"### {L['backtest_ref']}")
            for img_name in ["USDJPY_backtest.png", "EURUSD_backtest.png"]:
                img_path = REPORTS_DIR / img_name
                if img_path.exists():
                    with st.expander(img_name.replace("_", " ").replace(".png", "")):
